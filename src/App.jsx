...
import { User, Key, Sparkles, Copy, Plus, Trash2, ShieldCheck, Send, Loader2, Menu, X, MessageSquare, BookOpen, Terminal, Cpu, BarChart3, Database, Search, Wrench } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { supabase } from './supabaseClient';

const ADMIN_ID = 1562788488; // Твой ID
...
// --- Вкладка Админа (Новая) ---
const AdminView = ({ onBack }) => {
  const [stats, setStats] = useState({ users: 0, stars: 0, keys: 0 });
  ...
  const updateBalance = async (tgId, currentBalance, offset) => {
    const newBalance = currentBalance + offset;
    const { error } = await supabase.from('profiles').update({ stars_balance: newBalance }).eq('telegram_id', tgId);
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-6 space-y-6 pb-40 font-sans h-full overflow-y-auto no-scrollbar">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-black uppercase tracking-tighter text-red-500">Admin Panel</h2>
        <button onClick={onBack} className="p-2 glass-card text-gray-400"><X size={20}/></button>
      </div>

      {/* Статистика */}
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Юзеры', val: stats.users, icon: User },
          { label: 'Звезды', val: stats.stars.toFixed(0), icon: Sparkles },
          { label: 'Ключи', val: stats.keys, icon: Key },
        ].map((s, i) => (
          <div key={i} className="glass-card p-3 flex flex-col items-center border-white/5">
            <s.icon size={14} className="text-accent mb-1" />
            <span className="text-lg font-black">{s.val}</span>
            <span className="text-[8px] uppercase text-gray-500 font-bold">{s.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="relative">
        ...
        <input 
          ...
          className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-10 text-sm focus:outline-none focus:border-red-500/50"
        />
      </div>

      {/* Список пользователей */}
      <div className="space-y-3">
        {loading ? <Loader2 className="animate-spin mx-auto text-accent" /> : filteredUsers.map(u => (
          <div key={u.telegram_id} className="glass-card p-4 border-white/5 space-y-3">
            <div className="flex justify-between items-start">
              <div className="flex flex-col">
                <span className="font-bold text-sm">@{u.username || 'no_name'}</span>
                <span className="text-[10px] text-gray-500 font-mono">{u.telegram_id}</span>
              </div>
              <div className="text-yellow-500 font-black text-sm">{u.stars_balance?.toFixed(1)} ⭐</div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => updateBalance(u.telegram_id, u.stars_balance, 10)} className="flex-1 bg-green-500/10 hover:bg-green-500/20 text-green-500 py-2 rounded-lg text-[10px] font-bold uppercase transition-all">+10</button>
              <button onClick={() => updateBalance(u.telegram_id, u.stars_balance, 50)} className="flex-1 bg-green-500/10 hover:bg-green-500/20 text-green-500 py-2 rounded-lg text-[10px] font-bold uppercase transition-all">+50</button>
              <button onClick={() => updateBalance(u.telegram_id, u.stars_balance, -10)} className="flex-1 bg-red-500/10 hover:bg-red-500/20 text-red-500 py-2 rounded-lg text-[10px] font-bold uppercase transition-all">-10</button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 space-y-6 pb-32 font-sans">
      <div className="glass-card p-6 border-accent/20 bg-accent/5 relative text-center flex flex-col items-center">
          ...
          {isAdmin && (
            <button onClick={onOpenAdmin} className="mt-4 flex items-center gap-2 bg-red-500/20 text-red-500 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-red-500/30 active:scale-95 transition-all">
              <Wrench size={12}/> Админ-панель
            </button>
          )}
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {RECHARGE_PACKS.map(pack => (
            <button key={pack.amt} onClick={() => handlePayment(pack.amt)} className="glass-card p-4 flex flex-col items-center justify-center gap-2 border-white/5 active:scale-95 transition-all">
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          {AI_MODELS.map(m => (
            <div key={m.id} className="flex items-center gap-2.5 bg-accent/10 border border-accent/30 px-3 py-2 rounded-xl shadow-sm">
              <Cpu size={12} className="text-accent" /><span className="text-[10px] font-bold text-white uppercase tracking-tight">{m.name}</span>
              <div className="w-[1px] h-3 bg-white/10 mx-0.5" />
              <div className="flex items-center gap-1 text-yellow-400"><Sparkles size={10} fill="currentColor" /><span className="text-[10px] font-black tracking-tighter">{m.price}</span></div>
            </div>
          ))}
        </div>
      </header>
                  ...
                  code({node, inline, className, children, ...props}) {
                    return !inline ? (
                      <div className="relative my-2">
                        <pre className="bg-black/50 p-3 rounded-lg overflow-x-auto text-[11px] font-mono border border-white/5">{children}</pre>
                        <button onClick={() => navigator.clipboard.writeText(children)} className="absolute top-2 right-2 p-1 bg-white/5 rounded hover:bg-white/10"><Copy size={12} /></button>
                      </div>
      ...
      {currentChat && (
        <div className="fixed bottom-24 left-0 w-full p-4 bg-[#0a0a0a]/95 backdrop-blur-xl border-t border-white/5 z-40">
          <div className="relative max-w-md mx-auto flex gap-2">
            <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} placeholder="Напишите сообщение..." className="flex-1 bg-white/5 border border-white/10 rounded-2xl py-4 px-5 focus:outline-none focus:border-accent/50 text-sm shadow-inner text-white" />
        ...
        {isHistoryOpen && (
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed top-0 right-0 h-full w-4/5 bg-[#0a0a0a] z-[70] p-6 shadow-2xl border-l border-white/5">
              <div className="flex justify-between items-center mb-8 text-white"><h3 className="font-black uppercase tracking-widest text-xs">Архив диалогов</h3><X onClick={() => setIsHistoryOpen(false)} /></div>
...
const ApiView = ({ tgId }) => {
  ...
  return (
    <div className="p-6 space-y-6 pb-40 font-sans overflow-y-auto h-full no-scrollbar">
      <div className="flex justify-between items-center"><h2 className="text-2xl font-black tracking-tighter uppercase text-white">Разработчикам</h2><ShieldCheck className="text-green-500" /></div>
      <div className="glass-card p-5 border-white/5 space-y-4">
        <div className="flex items-center gap-2 text-accent font-black text-[10px] uppercase tracking-[0.2em] border-b border-white/5 pb-3"><BookOpen size={14} /> Полное руководство</div>
        <div className="space-y-4 text-[11px] text-gray-400 leading-relaxed">
          <div className="space-y-1">
            <span className="text-gray-100 font-bold block uppercase text-[9px] tracking-widest">Шаг 1: Endpoint</span>
            <code className="block bg-black/40 p-2.5 rounded-lg border border-white/5 text-accent text-[9px] whitespace-nowrap overflow-x-auto">https://{window.location.hostname}/api/chat</code>
          </div>
          <div className="space-y-1 pt-2">
            <span className="text-gray-100 font-bold block uppercase text-[9px] tracking-widest flex items-center gap-1"><Terminal size={10} /> Пример (cURL)</span>
            <pre className="bg-black p-4 rounded-xl text-[10px] text-green-500 overflow-x-auto border border-white/5 leading-snug font-mono">{`curl -X POST "${window.location.origin}/api/chat" \\
-H "Content-Type: application/json" \\
-d '{"apiKey": "KEY", "prompt": "Привет!", "messages": []}'`}</pre>
          </div>
        </div>
      </div>
      <button onClick={createKey} className="w-full glass-card border-dashed border-accent/40 p-5 flex items-center justify-center gap-3 text-accent font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all"><Plus size={18} /> Создать новый ключ</button>
      <div className="space-y-3">
        {keys.map(k => (
          <div key={k.id} className="glass-card p-4 border-white/5 flex flex-col space-y-2">
            ...
            <div className="flex items-center justify-between bg-black/40 rounded-xl px-3 py-2.5 border border-white/5 font-mono text-[10px] text-accent shadow-inner">
              <span className="truncate mr-4 uppercase">{k.key_value}</span>
              <Copy size={14} className="text-gray-500 hover:text-white cursor-pointer" onClick={() => { navigator.clipboard.writeText(k.key_value); alert("Ключ скопирован!"); }} />
            </div>
          </div>
        ))}
      </div>
